...
<head>
    <meta charset="UTF-8">
    <title>Purchase Order Event</title>
    <style>
...
<body>
    ...
    <div class="header">
        <h1>PURCHASE ORDER (PO) EVENT</h1>
        <p>Formulir Pengadaan Bahan Baku Khusus Event: <strong>{{ $event->nama_event }}</strong></p>
    </div>

    <table class="info-table">
        <tr>
            <td width="20%"><strong>No. PO</strong></td>
            <td width="30%">: PO-EVT-{{ date('Ymd') }}-{{ str_pad($event->id ?? 1, 4, '0', STR_PAD_LEFT) }}</td>
    ...
    <table class="item-table">
        <thead>
            <tr>
                ...
                <th width="15%" style="text-align: center;">Jumlah Beli</th>
                <th width="15%" style="text-align: center;">Satuan</th>
            </tr>
        </thead>
        <tbody>
            @foreach($detailsToPrint as $item) <!-- 🔥 Ubah variabelnya jadi ini 🔥 -->
                @if($item->jumlah_beli > 0)
                <tr>
                    <td style="text-align: center;">{{ $loop->iteration }}</td>
                    <td><strong>{{ $item->bahanBaku->nama_bahan ?? '-' }}</strong></td>
                    <td style="text-align: center; font-size: 16px; font-weight: bold;">{{ $item->jumlah_beli }}</td>
    ...
    <p style="font-size: 12px; color: #666; font-style: italic; background: #fdfaf5; padding: 10px; border-left: 3px solid #e67e22;">
        * Dokumen ini adalah surat jalan/pesanan resmi yang dikeluarkan oleh sistem setelah mendapat persetujuan (ACC) dari Operational Manager.<br>
        * Pengadaan ini dikhususkan untuk menunjang kebutuhan pelaksanaan event.
    </p>
